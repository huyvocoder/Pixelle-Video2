...
from web.utils.persist_inputs import load_last_inputs
...
def render_tts_section(pixelle_video, last):
    """Render TTS configuration section"""
    # TTS Section (moved from left column)
    # ====================================================================
    with st.expander(tr('section.tts'), expanded=False):
        ...
        saved_tts_mode = last.get("tts_inference_mode", tts_config.get("inference_mode", "local"))
        tts_mode = st.radio(
        ...
        if tts_mode == "local":
            ...
            saved_voice = last.get("tts_voice") or local_config.get("voice", "zh-CN-YunjianNeural")
            saved_speed = last.get("tts_speed") or local_config.get("speed", 1.2)
        ...
        # ================================================================
            ...
            default_tts_index = 0
            saved_tts_workflow = last.get("tts_workflow") or tts_config.get("comfyui", {}).get("default_workflow")
...
def render_template_section(pixelle_video, last):
    """Render Template configuration section"""
    # ====================================================================
    ...
    with st.expander(tr('section.template'), expanded=False):
        ...
        saved_template_type = last.get("selected_template_type", "image")
        selected_template_type = st.radio(
            ...
            index=template_type_keys.index(saved_template_type) if saved_template_type in template_type_keys else 1,
        ...
        if 'selected_template' not in st.session_state:
            # Prefer the template (ratio/size) the user picked last time, but only
            # if it still belongs to the currently selected template type.
            saved_frame_template = last.get("frame_template")
            valid_saved_paths = {
                t.template_path
                for templates in grouped_templates.values()
                for t in templates
            }
            if saved_frame_template and saved_frame_template in valid_saved_paths:
                st.session_state['selected_template'] = saved_frame_template
            else:
                st.session_state['selected_template'] = type_specific_default
            # Important: also seed last_template_type here. Otherwise the
            # type-change-detection block right below sees last_template_type
            # as None on the very first render of a new session (always
            # different from the current type) and immediately resets
            # selected_template back to the default, discarding what we
            # just restored above.
        ...
        saved_template_params = last.get("template_params") or {}
        if custom_params_for_video:
            ...
            with video_custom_col1:
                for param_name, config in param_items[:mid_point]:
                    ...
                    default = saved_template_params.get(param_name, config['default'])
                    label = config['label']
...
def render_workflow_section(pixelle_video, last):
    """Render Media Workflow configuration section"""
    # ====================================================================
    ...
    if template_requires_media:
        ...
        # Prefer the workflow the user actually picked last time over the config default.
        saved_workflow = last.get(f"media_workflow__{media_config_key}") or saved_workflow
        workflow_key = None
        ...
        with st.expander(section_title, expanded=False):
            ...
            # Prompt prefix input
            # Prefer the last value the user actually typed (persisted across sessions);
            # fall back to the config-defined default if nothing was saved yet.
            # NOTE: save_last_inputs() saves the whole video_params dict as-is,
            # which uses the flat keys "prompt_prefix" + "prompt_prefix_scope"
            # (see style_config's own return dict below) - NOT a scoped key
            # like "prompt_prefix__image". Read from the same flat keys here,
            # only using the saved value if it was saved for this same media type.
            saved_scope = last.get("prompt_prefix_scope")
            saved_prefix_value = last.get("prompt_prefix")
            if saved_scope == media_config_key and saved_prefix_value:
                current_prefix = saved_prefix_value
            else:
                current_prefix = comfyui_config.get(media_config_key, {}).get("prompt_prefix", "")
        
            # Prompt prefix input (persisted to data/last_inputs.json on Generate).
            # IMPORTANT: needs an explicit `key` - without one, Streamlit
            # recomputes `value=` from disk on every rerun (e.g. triggered by
            # touching ANY other widget on the page), wiping out whatever the
            # user just typed here before they got a chance to click Generate.
            # With a key, Streamlit keeps the in-progress edit in session_state
            # and only falls back to `value=` on the very first render of a
            # fresh session.
            prompt_prefix = st.text_area(
                ...
                key=f"prompt_prefix_input__{media_config_key}"
            )
    ...
    return {
        ...
        "media_workflow__image": final_media_workflow if media_config_key == "image" else last.get("media_workflow__image"),
        "media_workflow__video": final_media_workflow if media_config_key == "video" else last.get("media_workflow__video"),
        ...
        "prompt_prefix_scope": media_config_key,
        "media_width": media_width,
        "media_height": media_height
    }

def render_style_config(pixelle_video):
    """Render all style configuration sections (backward compatibility)"""
    last = load_last_inputs()
    
    tts_params = render_tts_section(pixelle_video, last)
    template_params = render_template_section(pixelle_video, last)
    workflow_params = render_workflow_section(pixelle_video, last)
    
    return {**tts_params, **template_params, **workflow_params}
