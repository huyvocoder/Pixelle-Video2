...
def render_sidebar_controls(pixelle_video):
    ...
    with st.sidebar:
        # Statistics
        st.markdown(f"**:material/bar_chart: {tr('history.total_tasks')}**")
        ...
        # Filters
        st.markdown(f"**:material/filter_list: {tr('history.filter_status')}**")
        status_options = {
        ...
        # Sort
        st.markdown(f"**:material/sort: {tr('history.sort_by')}**")
...
def render_grid_task_card(task: dict, pixelle_video):
    ...
    status_map = {
        ...
        "running": ":material/hourglass_empty:",
        "pending": ":material/pause_circle:",
    }
    status_icon = status_map.get(status, ":material/help:")
    ...
    with st.container():
        ...
        with col1:
            if st.button(":material/visibility:", key=f"view_{task_id}", help=tr("history.task_card.view_detail"), use_container_width=True):
        ...
        with col2:
            if video_path and os.path.exists(video_path):
                ...
                st.button(":material/download:", key=f"download_disabled_{task_id}", disabled=True, use_container_width=True)
        
        with col3:
            if st.button(":material/delete:", key=f"delete_{task_id}", help=tr("history.task_card.delete"), use_container_width=True):
...
def render_task_detail_modal(task_id: str, pixelle_video):
    ...
    if st.button(":material/close: " + tr("history.detail.close"), key=f"close_detail_top_{task_id}"):
    ...
    if st.button(":material/close: " + tr("history.detail.close"), key=f"close_detail_bottom_{task_id}"):
        st.session_state[f"detail_{task_id}"] = False
        st.rerun()
...
def main():
    ...
    if total_pages > 1:
        ...
        with col1:
            if st.button(":material/arrow_back: Previous", disabled=st.session_state.history_page == 1, use_container_width=True):
                st.session_state.history_page -= 1
                st.rerun()
        ...
        with col3:
            if st.button("Next :material/arrow_forward:", disabled=st.session_state.history_page == total_pages, use_container_width=True):
                st.session_state.history_page += 1
                st.rerun()
