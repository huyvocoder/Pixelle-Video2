...
async def generate_narrations_from_topic(
    ...
    Args:
        ...
        narrative_mode: "standard" (default) or "character_story" (tell the
                         topic through one invented relatable protagonist's
                         journey instead of direct listicle-style advice)
        batch_size: Max narrations requested per single LLM call. For large
                    n_scenes (e.g. 400 for long videos), a single call risks
                    hitting token/context limits and JSON truncation - so
                    requests are split into multiple calls of at most this
                    size, chained with continuation context so long-form
                    story mode stays coherent across batches.
    ...
    logger.info(f"Generating {n_scenes} narrations from topic: {topic} (narrative_mode={narrative_mode})")
    
    if n_scenes <= batch_size:
        # Small/typical case: single call, unchanged behavior.
        return await _generate_narration_batch(
            llm_service, topic, n_scenes, min_words, max_words, narrative_mode
        )
    
    # Large case: chain multiple batches, carrying continuation context
    # forward so tone/protagonist stays consistent and to reduce the
    # chance any single call fails (only that batch needs retrying, not
    # the whole 400-scene request).
    ...
    while remaining > 0:
        ...
        if all_narrations:
            # Give the LLM the last few narrations as context, not the
            # entire growing list, to keep each call's prompt bounded.
            recent = all_narrations[-5:]
            continuation_context = "\n".join(f"- {n}" for n in recent)
        
        logger.info(f"  Batch {batch_num}: generating {this_batch_size} narrations "
                    f"({len(all_narrations)}/{n_scenes} done so far)")
        
        batch_narrations = await _generate_narration_batch(
            llm_service, topic, this_batch_size, min_words, max_words,
            narrative_mode, continuation_context
        )
        all_narrations.extend(batch_narrations)
        remaining -= this_batch_size
    
    logger.info(f"✅ Generated {len(all_narrations)} narrations across {batch_num} batches")
    return all_narrations


async def _generate_narration_batch(
    llm_service,
    topic: str,
    n_storyboard: int,
    min_words: int,
    ...
    """Single LLM call generating up to n_storyboard narrations. Internal
    helper used by generate_narrations_from_topic for both the simple
    single-call path and each chunk of the batched path."""
    from pixelle_video.prompts import build_topic_narration_prompt
    ...
    # Scale max_tokens to the batch size instead of a fixed value that
    # would silently truncate output (and break JSON parsing) once
    # n_storyboard gets large. ~120 tokens/narration is a generous buffer
    # for max_words up to ~40-50 words plus JSON formatting overhead.
    max_tokens = min(8000, max(2000, n_storyboard * 120))
    ...
    # Validate count
    if len(narrations) > n_storyboard:
        logger.warning(f"Got {len(narrations)} narrations, taking first {n_storyboard}")
        narrations = narrations[:n_storyboard]
    elif len(narrations) < n_storyboard:
        raise ValueError(f"Expected {n_storyboard} narrations, got only {len(narrations)}")
