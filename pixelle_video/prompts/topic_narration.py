...
CHARACTER_STORY_BLOCK = """## Character-Driven Storytelling Mode (ACTIVE - highest priority structural requirement)
Instead of stating advice, facts, or tips directly to the audience, tell this entire topic through the eyes of ONE specific, relatable hypothetical PROTAGONIST that you invent for this video.
- Invent concrete, relatable details for the protagonist appropriate to the topic (e.g. "a 22-year-old fresh graduate", "a tired office worker in his 30s", "a first-time parent") - pick whichever age/situation makes the topic feel most relatable, not necessarily always the same demographic
- Storyboard 1 (hook): introduce this protagonist and put the audience directly in their shoes - e.g. "Imagine you're a 22-year-old..." or open on the protagonist's relatable situation/struggle/question that connects to the topic
- Middle storyboards: narrate the protagonist's journey - their decisions, mistakes, discoveries, or turning points - that naturally reveal the topic's key points through what happens to them, rather than stating the points as direct advice. Show consequences and realizations through the protagonist's experience.
- Last storyboard: resolve through what the protagonist learned or gained, tying it back to a concrete, actionable takeaway the audience can apply themselves
- The PROTAGONIST (identity and narrative perspective) must stay the SAME across all storyboards - do not switch to a different person's perspective mid-video
- Supporting characters ARE allowed when the story naturally calls for them (e.g. a friend, mentor, colleague, family member the protagonist talks to or is influenced by in a specific scene) - this is normal and encouraged for a believable story. Just keep it clear which one is the protagonist across the whole narrative.
- The narration should still be spoken in second person or narrator voice as usual for TTS (e.g. "she realized..." or "you start to notice...") - this is a storytelling STRUCTURE, not a change to who is speaking the narration
- All other requirements below (opening diversity, no repeated opening words, word count, tone) still fully apply on top of this structure
...
{character_story_block}## Content Structure Suggestions
...
def build_topic_narration_prompt(
    ...
    Args:
        ...
        narrative_mode: "standard" (default listicle/viewpoint style) or
                         "character_story" (tell the topic through one
                         invented relatable character's journey instead of
                         direct advice/tips)
        continuation_context: For very long videos generated in batches -
                         pass in the last few narrations already generated
                         so this batch continues the same tone/protagonist
                         and doesn't repeat opening words already used.
                         Leave empty for a normal single-call generation.
    
    Returns:
        Formatted prompt
    """
    character_story_block = CHARACTER_STORY_BLOCK if narrative_mode == "character_story" else ""

    continuation_block = ""
    if continuation_context:
        continuation_block = f"""## Continuation Context (IMPORTANT - this is not the first batch)
This is a CONTINUATION of a longer video already in progress. Here are the most recent narrations already generated - continue the SAME tone, and if in character-story mode, the SAME protagonist and ongoing situation. Do not restart the story or introduce a new protagonist. Also do not reuse any opening word/phrase already used below:

{continuation_context}

Continue naturally from here for the next {n_storyboard} storyboards.
...
"""
    ...
    return TOPIC_NARRATION_PROMPT.format(
        ...
        min_words=min_words,
        max_words=max_words,
        character_story_block=continuation_block + character_story_block
    )
